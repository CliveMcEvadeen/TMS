...
class PaymentController extends Controller
{
    ...
    {
        try {
            $payments = Payment::where('user_id', Auth::id())->get(); // Changed tenant_id to user_id
    ...
    {
        try {
            $payments = Payment::with('tenant')->get(); // Ensure this relationship is defined in the Payment model
    ...
    {
        try {
            $payment = Payment::where('payment_id', $id)->firstOrFail(); // Changed to use payment_id
            ...
            Log::info('Payment verified successfully', ['payment_id' => $payment->payment_id]);
    ...
    public function reject($id)
    {
        try {
            ...
            Log::info('Payment rejected successfully', ['payment_id' => $payment->payment_id]);
    ...
    {
        try {
            $payment = Payment::where('payment_id', $paymentId) // Changed to use payment_id
                ->where('payment_status', 'approved')
                ->with('tenant')
                ->firstOrFail();
